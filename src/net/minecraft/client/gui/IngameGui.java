...
public class IngameGui extends AbstractGui {
    ...
    public void renderIngameGui(MatrixStack matrixStack, float partialTicks) {
        ...
        if (conduction.FUNCTION_MANAGER.hud.elements.get(7)) {
        ...
        //HudFunction hudFunction = conduction.FUNCTION_MANAGER.hudFunction;
...
//        }
    ...
    private void func_238447_a_(MatrixStack matrixstack, ScoreObjective p_238447_2_) {
       ...
       if (ClientUtils.legitMode) {
           ...
           RenderUtils.Render2D.drawRoundedCorner(l2 - 2, j2 - i2+2, scaledWidth - l2, i2, new Vector4f(0, 4, 0, 4), ColorUtil.rgba(0, 0, 0, 80));
           ...
           for (Pair<Score, ITextComponent> pair : list1) {
               ...
               this.getFontRenderer().func_243248_b(matrixstack, itextcomponent2, (float) l2, (float) k1, -1);
               if (conduction.FUNCTION_MANAGER != null && conduction.FUNCTION_MANAGER.nameProtect.state) {
                   this.getFontRenderer().drawString(matrixstack, conduction.FUNCTION_MANAGER.nameProtect.patch(s), (float) (l1 - this.getFontRenderer().getStringWidth(s)), (float) k1, -1);
               } else
                   this.getFontRenderer().drawString(matrixstack, s, (float) (l1 - this.getFontRenderer().getStringWidth(s)), (float) k1, -1);

               if (l == collection.size()) {

                   RenderUtils.Render2D.drawRoundedCorner(l2 - 2, k1 - 10, scaledWidth - l2, 10, new Vector4f(4, 0, 4, 0), ColorUtil.rgba(0, 0, 0, 20));

                   fill(matrixstack, l2 - 2, k1 - 9 - 1, l1, k1 - 1, j1);
                   fill(matrixstack, l2 - 2, k1 - 1, l1, k1, i1);
                   this.getFontRenderer().func_243248_b(matrixstack, itextcomponent, (float) (l2 + j / 2 - i / 2), (float) (k1 - 9), -1);
               }
           }
        }
       else {
           for (Pair<Score, ITextComponent> pair : list1) {
               ...
               RenderUtils.Render2D.drawRoundOutline(l2-3, j2 - i2, scaledWidth - l2, i2, 4, 0.01f, ColorUtil.rgba(0, 0, 0, 0), new Vector4i(
                       RenderUtils.reAlphaInt(conduction.STYLE_MANAGER.getCurrentStyle().getColor(10), (int) (255 * 1)),
                       RenderUtils.reAlphaInt(conduction.STYLE_MANAGER.getCurrentStyle().getColor(50), (int) (255 * 1)),
                       RenderUtils.reAlphaInt(conduction.STYLE_MANAGER.getCurrentStyle().getColor(100), (int) (255 * 1)),
                       RenderUtils.reAlphaInt(conduction.STYLE_MANAGER.getCurrentStyle().getColor(150), (int) (255 * 1))));
               RenderUtils.Render2D.drawRoundedCorner(l2-3, j2 - i2, scaledWidth - l2, i2, 4, ColorUtil.rgba(15, 15, 15, 10));
