...
public class ChatScreen extends Screen {
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        boolean hiderpass = conduction.FUNCTION_MANAGER.hider.state; //создайте класс PasswordHider
            ...
            boolean reg4 = this.inputField.getText().startsWith("/register");

            boolean tpa1 = this.inputField.getText().startsWith("/call");
            boolean tpa2 = this.inputField.getText().startsWith("/tpa");
            boolean tpa3 = this.inputField.getText().startsWith("/tp");
            ...
            if (hiderpass) {
                if (conduction.FUNCTION_MANAGER.hider.pass.get()) {
                if (reg1 || reg2 || reg3 || reg4) {
                    ...
                    int height = 12;
                    int radius = 1;
                    int color = new java.awt.Color(0, 0, 0, 255).getRGB();
                    RenderUtils.Render2D.drawRoundedRect(X, Y - 1, width - 700, height, radius, color);
                }
            }
                if (conduction.FUNCTION_MANAGER.hider.teleport.get()) {
                    if (tpa1 || tpa2 || tpa3) {
                        int X = 2;
                        int Y = this.height - 13;
                        int width = this.width - 4;
