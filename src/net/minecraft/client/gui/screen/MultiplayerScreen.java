...
import pa.centric.client.ui.proxy.ProxyUI;
...
public class MultiplayerScreen extends Screen
{
    ...
    protected void init()
    {
        ...
        ServerData holyworld = new ServerData("holyworld", "mc.holyworld.ru", true);
        ServerData reallyworld = new ServerData("reallyworld", "mc.reallyworld.ru", true);
        MultiplayerScreen multiplayerScreen = (MultiplayerScreen) Minecraft.getInstance().currentScreen;
        assert multiplayerScreen != null;
        if (!ClientUtils.legitMode) {
            this.addButton(new Button(this.width / 2 + 4 + 50 + 105, this.height - 52, 100, 20, new TranslationTextComponent("HolyWorld"), (ppp) ->
            {
                multiplayerScreen.connectToServer(holyworld);
            }));
        }
        if (!ClientUtils.legitMode) {
            this.addButton(new Button(this.width / 2 + 4 + 50 + 105, this.height - 28, 100, 20, new TranslationTextComponent("ReallyWorld"), (ppp) ->
            {
                multiplayerScreen.connectToServer(reallyworld);
            }));
        }
