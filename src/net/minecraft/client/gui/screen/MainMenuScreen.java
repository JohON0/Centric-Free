...
public class MainMenuScreen extends Screen {
    ...
    public void init(Minecraft minecraft, int width, int height) {
        ...
        int buttonWidth = (int) ((int) (333 / 2f));
        int buttonHeight = (int) ((int) (48 / 2f));
        int off = (int) (buttonHeight + 5);
        this.addButton(new Button((int) (mc.getMainWindow().scaledWidth() / 2 - buttonWidth / 2f), mc.getMainWindow().scaledHeight() / 2  - 15, buttonWidth, buttonHeight, new StringTextComponent("Одиночная игра"), p_onPress_1_ -> {
            mc.displayGuiScreen(new WorldSelectionScreen(this));
        }));
        this.addButton(new Button((int) (mc.getMainWindow().scaledWidth() / 2 - buttonWidth / 2f), mc.getMainWindow().scaledHeight() / 2 - 15 + off, buttonWidth, buttonHeight, new StringTextComponent("Сетевая игра"), p_onPress_1_ -> {
            mc.displayGuiScreen(new MultiplayerScreen(this));
        }));
        this.addButton(new Button((int) (mc.getMainWindow().scaledWidth() / 2 - buttonWidth / 2f), mc.getMainWindow().scaledHeight() / 2  - 15 + off * 2, buttonWidth, buttonHeight, new StringTextComponent("Аккаунты"), p_onPress_1_ -> {
            mc.displayGuiScreen(conduction.ALT);
        }));
        this.addButton(new Button((int) (mc.getMainWindow().scaledWidth() / 2 - buttonWidth / 2f), mc.getMainWindow().scaledHeight() / 2  - 15 + off * 3, buttonWidth, buttonHeight, new StringTextComponent("Настройки"), p_onPress_1_ -> {
            mc.displayGuiScreen(new OptionsScreen(this, this.minecraft.gameSettings));
        }));
        this.addButton(new Button((int) (mc.getMainWindow().scaledWidth() / 2 - buttonWidth / 2f), mc.getMainWindow().scaledHeight() / 2  - 15 + off * 4, buttonWidth-90, buttonHeight, new StringTextComponent("Дискорд"), p_onPress_1_ -> {
            String url = "http://discord.gg/tA2XQpWfvB";
            try {
        ...
        this.addButton(new Button((int) (mc.getMainWindow().scaledWidth() / 2 - buttonWidth / 2f)+90, mc.getMainWindow().scaledHeight() / 2  - 15 + off * 4, buttonWidth-90, buttonHeight, new StringTextComponent("ВКонтакте"), p_onPress_1_ -> {
            String url = "http://vk.com/centricclient";
            try {
                Runtime.getRuntime().exec("rundll32 url.dll,FileProtocolHandler " + url);
            } catch (Exception e) {
                System.out.println("Ошибка при открытии браузера: " + e.getMessage());
        ...
        this.addButton(new Button((int) (mc.getMainWindow().scaledWidth() / 2 - buttonWidth / 2f+38), (int) (mc.getMainWindow().scaledHeight() / 2  - 15 + 250), 100, 24, new StringTextComponent("Выход"), p_onPress_1_ -> {
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        @author JohON0
        */
        float xlogocenter = mc.getMainWindow().scaledWidth() / 2f;
        RenderUtils.Render2D.drawImage(new ResourceLocation("centric/images/backmenu.png"), 0, 0, mc.getMainWindow().scaledWidth(), mc.getMainWindow().scaledHeight(), -1);
        FontUtils.iconlogo[130].drawCenteredString(matrixStack,"A", xlogocenter, mc.getMainWindow().scaledHeight() / 2f - 95, -1);
        super.render(matrixStack, mouseX, mouseY, partialTicks);
        FontUtils.sfbold[16].drawString(matrixStack,"Авторское право Centric Client 2024",mc.getMainWindow().scaledWidth() / 2f - FontUtils.sfbold[11].getWidth("Авторское право Centric Client 2024") / 2f-20, (mc.getMainWindow().scaledHeight()) / 2f - 240, ColorUtil.rgba(200,200,200,100));
    ...
    public static class Button extends AbstractButton {
        ...
        public void renderButton(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
            ...
            RenderUtils.Render2D.drawShadow(x, y, width, height, 5, ColorUtil.rgba(25,25,25,255));
            RenderUtils.Render2D.drawRoundedCorner(x, y, width, height,5,ColorUtil.rgba(25,25,25,255));
            GaussianBlur.startBlur();
            GaussianBlur.endBlur(20 + (animation * 10), 2);

            BloomHelper.registerRenderCall(() -> {
                FontUtils.sfbold[20].drawCenteredString(matrixStack, this.getMessage().getString(), x + width / 2f, y + height / 2f - FontUtils.sfbold[23].getFontHeight() / 2f + 4, RenderUtils.reAlphaInt(ColorUtil.interpolateColor(ColorUtil.getColorStyle(100), ColorUtil.rgba(255, 255, 255,255), animation), (int) (255 * animation)));

            });

            FontUtils.sfbold[20].drawCenteredString(matrixStack, this.getMessage().getString(), x + width / 2f, y + height / 2f - FontUtils.sfbold[23].getFontHeight() / 2f + 4, RenderUtils.reAlphaInt(ColorUtil.interpolateColor(ColorUtil.rgba(200, 200, 200,255), -1, animation), (int) (255)));
        }
