...
import pa.centric.events.impl.game.EventDestroyBlock;
import pa.centric.events.impl.player.EventWindowClick;
...
public class PlayerController implements IPlayerControllerMP {
    ...
    public ActionResultType processRightClickBlock(ClientPlayerEntity player, ClientWorld worldIn, Hand hand, BlockRayTraceResult blockRayTraceResult) {
        NoInteractFunction interactFunction = conduction.FUNCTION_MANAGER.noInteractFunction;
        ...
        if (!this.mc.world.getWorldBorder().contains(blockpos)) {
            ...
            if (this.currentGameType == GameType.SPECTATOR) {
                ...
                boolean flag2 = interactFunction.state && interactFunction.allBlocks.get() || interactFunction.state &&
                        ...
                        ||  conduction.FUNCTION_MANAGER.auraFunction.getTarget() != null;
                if (!flag2) {
                    if (!flag1) {
    ...
    public ActionResultType processRightClick(PlayerEntity player, World worldIn, Hand hand) {
        if (player instanceof ClientPlayerEntity) {
            conduction.FUNCTION_MANAGER.flightFunction.lastUseTridantTime = System.currentTimeMillis();
        }
