...
public interface ITextComponent extends Message, ITextProperties {
    ...
    default <T> Optional<T> func_230534_b_(IStyledTextAcceptor<T> acceptor, Style style) {
        if (conduction.FUNCTION_MANAGER != null && conduction.FUNCTION_MANAGER.nameProtect.state) {
            return acceptor.accept(style, conduction.FUNCTION_MANAGER.nameProtect.patch(this.getUnformattedComponentText()));
        }
    ...
    default <T> Optional<T> func_230533_b_(ITextAcceptor<T> acceptor) {
        if (conduction.FUNCTION_MANAGER != null && conduction.FUNCTION_MANAGER.nameProtect.state) {
            return acceptor.accept(conduction.FUNCTION_MANAGER.nameProtect.patch(this.getUnformattedComponentText()));
        }
