...
import wtf.expensive.util.Rotation;
import wtf.expensive.util.RotationHandler;
import wtf.expensive.util.math.*;
...
public class AuraFunction extends Function {
    ...
    private final ModeSetting rotationMode = new ModeSetting("Мод ротации", "Обычная", "Обычная", "FunTime", "ReallyWorld");
    ...
    private Vector3d getVector3d(LivingEntity me, LivingEntity to) {
        ...
        double yExpand = clamp(me.getPosYEye() - to.getPosY(), 0, to.getHeight() * (mc.player.getDistance(to) / (distance.getValue().floatValue())));
    ...
    private void updateRotation(float speed) {
        ...
        float yawDelta = wrapDegrees(rawYaw - rotate.x);
        float pitchDelta = wrapDegrees(rawPitch - rotate.y);
        ...
        float yaw = rotate.x + additionYaw + ThreadLocalRandom.current().nextFloat(-1f, 1);
        float pitch = rotate.y + additionPitch + ThreadLocalRandom.current().nextFloat(-1, 1);

        rotate.x = getSensitivity(yaw);
        rotate.y = getSensitivity(clamp(pitch, -89.0F, 89.0F));
    }
    ...
    Vector3d vec = target.getPositionVec().add(0, clamp(mc.player.getPosYEye() - target.getPosY(),
                    0, target.getHeight() * (mc.player.getDistanceToEntityEye(target) / distance.getValue().floatValue())), 0)
    ...
    private void attackAndRotateOnEntity(LivingEntity target) {
        ...
        switch (rotationMode.getIndex()) {
            case 0 -> {
                ...
                if (shouldAttack(target) && RayTraceUtil.getMouseOver(target, mc.player.rotationYaw + clampedYaw,
                        mc.player.rotationPitch + (mc.objectMouseOver.getType() == RayTraceResult.Type.ENTITY ? 0 : clampedPitch), distance.getValue().floatValue()) == target
    ...
    private void snapsRotation() {
        ...
        Vector3d vec = target.getPositionVec().add(0, MathHelper.clamp(mc.player.getEyePosition(mc.getRenderPartialTicks()).y - target.getPosY(),
                        0, target.getHeight() * (AuraUtil.getStrictDistance(target) / Math.max(mc.playerController.extendedReach() ? 6 : 3, distance.getValue().floatValue()))), 0)
