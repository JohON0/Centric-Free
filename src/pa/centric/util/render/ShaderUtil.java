...
import pa.centric.util.misc.FileUtil;
...
public class ShaderUtil implements IMinecraft {
    ...
    public ShaderUtil(String fragmentShaderLoc) {
        ...
        try {
            ...
            int fragmentShaderID = switch (fragmentShaderLoc) {
                        ...
                        createShader(new ByteArrayInputStream("""
                                ...
                                void main() {
                                    ...
                                    for (float f = 1.0; f <= radius; f++) {
                                        vec2 offsetDir = f * offset;
                                        vec3 sample1 = texture2D(textureIn, uv + offsetDir).rgb;
                                        vec3 sample2 = texture2D(textureIn, uv - offsetDir).rgb;
                                                              
                                        color += sample1 * weights[int(abs(f))];
                                        color += sample2 * weights[int(abs(f))];
                                                              
                                        totalWeight += 2.0 * weights[int(abs(f))];
                                    }
                ...
                case "blur3" ->
                        createShader(new ByteArrayInputStream("""
                                ...
                                uniform vec2 texelSize, direction;
                                uniform float radius, weights[256];
                                uniform vec3 overlayColor; // Добавлен uniform для цвета overlay
                                ...
                                void main() {
                                    ...
                                    vec3 blurredColor = color / totalWeight; // Рассчитанный цвет блюра
                                                               
                                    // Применяем overlay к рассчитанному цвету блюра
                                    vec4 overlay = vec4(overlayColor, 1.0);
                                    vec3 finalColor = mix(blurredColor, overlay.rgb, overlay.a);
                ...
                default ->
                        createShader(mc.getResourceManager().getResource(new ResourceLocation("centric/shader/" + fragmentShaderLoc)).getInputStream(), GL_FRAGMENT_SHADER);
            };
            ...
            System.out.println(Centric.prefix + "Ошибка при загрузке: " + fragmentShaderLoc);
        }
    }
