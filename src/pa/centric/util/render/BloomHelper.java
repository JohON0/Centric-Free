...
import static pa.centric.util.render.OutlineUtils.setupBuffer;
import static pa.centric.util.render.ShaderUtil.calculateGaussianValue;
...
public class BloomHelper {
    ...
    public static void draw(int radius, float exp, boolean fill) {
        if (renderQueue.isEmpty() || (conduction.FUNCTION_MANAGER.optimization.state && conduction.FUNCTION_MANAGER.optimization.optimizeSelection.get(2)))
            return;
