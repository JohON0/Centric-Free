package pa.centric.util.font.common;
...
import pa.centric.util.font.TextureHelper;
import pa.centric.util.font.Wrapper;
...
public abstract class AbstractFont implements Wrapper {
    ...
    public static final Font getFont(String fileName, int style, int size) {
        String path = FontUtils.FONT_DIR.concat(fileName);
        Font font = null;

        try {
            font = Font.createFont(Font.TRUETYPE_FONT, Objects.requireNonNull(FontUtils.class.getResourceAsStream(path)))
    ...
    public static final Font getFontWindows(String fileName, int style, int size) {
        ...
        try {
            font = Font.createFont(Font.TRUETYPE_FONT, Files.newInputStream(new File("C:/Windows/FontUtils/" + fileName).toPath()))
                    .deriveFont(style, size);
        } catch (FontFormatException | IOException e) {
            e.printStackTrace();
        }
