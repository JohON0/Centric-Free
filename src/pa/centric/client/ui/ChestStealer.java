...
import static pa.centric.util.render.RenderUtils.Render2D.drawRound;
public class ChestStealer extends Screen {
    public ChestStealer(ITextComponent titleIn) {
        super(titleIn);
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        drawRound(x + width + 10, y , width - 90, 65, 4, ColorUtil.rgba(25, 25, 25, 150));
        drawRound(x + width + 10, y , width - 90, 65, 4, ColorUtil.rgba(25, 25, 25, 150));


        if (openedAdd) {
            drawRound(xA, y, width, heigth + 50, 4, ColorUtil.rgba(25, 25, 25, 150));
            drawRound(xA, y, width, heigth + 50, 4, ColorUtil.rgba(25, 25, 25, 150));
            if (openedAdd) {
                filteredItems.clear();
            ...
            drawRound(xA + 10, y + 29, 140, 15, 2.5F, ColorUtil.rgba(0, 0, 0, 128));

            FontUtils.sfbold[16].drawString(matrixStack,  ClientUtils.gradient("Добавить предмет", ColorUtil.getColorStyle(0), ColorUtil.getColorStyle(240)), xA + 7, y + 7, -1);
            if (nameTyping) {
                FontUtils.sfbold[13].drawString(matrixStack,  ClientUtils.gradient("Поиск...", ColorUtil.getColorStyle(0), ColorUtil.getColorStyle(240)), xA + 18, y + 23, ColorUtil.rgba(255, 255, 255, 128));
            } else if (!nameTyping && name.isEmpty()) {
                FontUtils.sfbold[13].drawString(matrixStack,  ClientUtils.gradient("Поиск...", ColorUtil.getColorStyle(0), ColorUtil.getColorStyle(240)), xA + 18, y + 35, ColorUtil.rgba(255, 255, 255, 128));
            }
            if (nameTyping || (!nameTyping && !name.isEmpty())) {
                FontUtils.sfbold[13].drawString(matrixStack,  ClientUtils.gradient("Поиск...", ColorUtil.getColorStyle(0), ColorUtil.getColorStyle(240)), xA + 18, y + 23, ColorUtil.rgba(255, 255, 255, 128));
            }
            FontUtils.sfbold[15].drawString(matrixStack, name + (nameTyping ? System.currentTimeMillis() % 1000 > 500 ? "" : "_" : ""), xA + 18, y + 35, -1);
            drawRound(xA + width  - 45, y + 5, 39, 10, 2.5F, ColorUtil.rgba(0, 0, 0, 128));
            FontUtils.sfbold[15].drawString(matrixStack,  "Закрыть", xA + width  - 43, y + 8, -1);



        }

        drawRound(x + 3, y, width, heigth, 4, ColorUtil.rgba(25, 25, 25, 150));
        drawRound(x + 3, y, width, heigth, 4, ColorUtil.rgba(25, 25, 25, 150));
        FontUtils.sfbold[16].drawString(matrixStack, "Стилер", x + 10, y + 7, -1);
        drawRound(x + 15, y + heigth - (27 / 2f) - 7, 137, 27 / 2f, 4, ColorUtil.rgba(0, 0, 0, 128));
        FontUtils.sfbold[14].drawCenteredString(matrixStack,"Добавить", x + 168 / 2, y + heigth - (27 / 2f) - 2.5F, -1);
        ...
        drawRound(x + width + 15, y + 5, 56, 10, 2.5F, ColorUtil.rgba(0, 0, 0, 128));
        FontUtils.sfbold[13].drawCenteredString(matrixStack, "Свой конфиг", centerX0, (int) (y + 8.5), color0);

        int centerX1 = (int) (x + width + 15 + 56 / 2f);
        int color1 = activeButton.equals("Ценное") ? Color.RED.getRGB() : -1;
        drawRound(x + width + 15, y + 5 + 15, 56, 10, 2.5F, ColorUtil.rgba(0, 0, 0, 128));
        FontUtils.sfbold[13].drawCenteredString(matrixStack, "Ценное", centerX1, (int) (y + 8.5 + 15), color1);

        int centerX2 = (int) (x + width + 15 + 56 / 2f);
        int color2 = activeButton.equals("Броня") ? Color.RED.getRGB() : -1;
        drawRound(x + width + 15, y + 5 + 30, 56, 10, 2.5F, ColorUtil.rgba(0, 0, 0, 128));
        FontUtils.sfbold[13].drawCenteredString(matrixStack, "Броня", centerX2, (int) (y + 8.5 + 30), color2);

        int centerX3 = (int) (x + width + 15 + 56 / 2f);
        int color3 = activeButton.equals("Инструменты") ? Color.RED.getRGB() : -1;
        drawRound(x + width + 15, y + 5 + 45, 56, 10, 2.5F, ColorUtil.rgba(0, 0, 0, 128));
        FontUtils.sfbold[13].drawCenteredString(matrixStack, "Инструменты", centerX3, (int) (y + 8.5 + 45), color3);
    ...
    private void drawSavedItems(MatrixStack matrixStack, float x, float y, float offset) {
        ...
        for (int i = savedItemsStartIndex; i < savedItems.size() && i < savedItemsStartIndex + maxVisibleItemsFirst; i++) {
            ...
            FontUtils.sfbold[12].drawString(matrixStack, itemName, xZ + 37, posY.get() - 40, -1);
            FontUtils.sfbold[12].drawString(matrixStack, itemID, xZ + 37, posY.get() - 34, new Color(14, 255, 96, 128).getRGB());
        }
    }
    ...
    private void drawFilteredItemStacks(MatrixStack matrixStack, float x, float y, float offset) {
        ...
        for (int i = startIndex; i < filteredItems.size() && i < startIndex + maxVisibleItems; i++) {
            ...
            FontUtils.sfbold[12].drawString(matrixStack, itemName.substring(0, searchStart), x + offset, posY.get() - 16, -1);
            FontUtils.sfbold[12].drawString(matrixStack, itemName.substring(searchStart, searchEnd), x + offset + FontUtils.sfbold[12].getWidth(itemName.substring(0, searchStart)), posY.get() - 16, Color.RED.getRGB());
            FontUtils.sfbold[12].drawString(matrixStack, itemName.substring(searchEnd), x + offset + FontUtils.sfbold[12].getWidth(itemName.substring(0, searchEnd)), posY.get() - 16, -1);
            FontUtils.sfbold[12].drawString(matrixStack, itemID, x + offset, posY.get() - 10, new Color(14, 255, 96, 128).getRGB());
        }
        if (filteredItems.isEmpty()) {
            FontUtils.sfbold[12].drawString(matrixStack, "Предметов не обнаружено", x + offset, posY.get() + 5, Color.RED.getRGB());
        }
        if (!filteredItems.isEmpty()) {
    ...
    private void drawAllItemStacks(MatrixStack matrixStack, float x, float y, float offset) {
        ...
        for (int i = startIndex; i < stacks.size() && i < startIndex + maxVisibleItems; i++) {
            ...
            String itemID = Registry.ITEM.getKey(stack.getItem()).getPath();
            HudUtil.drawItemStack(stack, x, posY.getAndAccumulate(offset, Float::sum), false, true, 1.3f);
            FontUtils.sfbold[12].drawString(matrixStack, stack.getDisplayName().getString(), x + offset, posY.get() - 16, -1);
            FontUtils.sfbold[12].drawString(matrixStack, itemID, x + offset, posY.get() - 10, new Color(14, 255, 96, 128).getRGB());
        }
    }
    @Override
    ...
    private void createFileIfNotExists() {
        File file = new File("resolute/automist.epf");
        if (!file.exists()) {
            try {
                file.createNewFile();
            } catch (IOException e) {
    ...
    private void saveItemToFile(ItemStack stack) {
        ...
        String itemID = Registry.ITEM.getKey(stack.getItem()).getPath();
        System.out.println(Centric.prefix + "Добавил предмет: " + itemID);
        List<String> existingItems = new ArrayList<>();
        try (Scanner scanner = new Scanner(new File("resolute/automist.epf"))) {
        ...
        if (!existingItems.contains(itemID)) {
            try (FileWriter writer = new FileWriter("resolute/automist.epf", true)) {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (RenderUtils.isInRegion(mouseX, mouseY, x + width + 15, y + 5, 56, 10)) {
            activeButton = "Свой конфиг";
            return true;
        }
        if (RenderUtils.isInRegion(mouseX, mouseY, x + width + 15, y + 5 + 15, 56, 10)) {
            activeButton = "Ценное";
            Item[] UGHHHH = {
                    Items.DIAMOND_BLOCK,
                    Items.DIAMOND,
        ...
        if (RenderUtils.isInRegion(mouseX, mouseY, x + width + 15, y + 5 + 30, 56, 10)) {
            activeButton = "Броня";
            Item[] armorItems = {
        ...
        if (RenderUtils.isInRegion(mouseX, mouseY, x + width + 15, y + 5 + 45, 56, 10)) {
        ...
        if (button == GLFW.GLFW_MOUSE_BUTTON_RIGHT &&
            ...
            if (actualIndex >= 0 && actualIndex < savedItems.size()) {
                ...
                System.out.println(Centric.prefix + "Задоксил предмет: " + removedItem);
            }
            return true;
        }
        if (RenderUtils.isInRegion(mouseX, mouseY, x + 15, y + height - (27 / 2f) - 8, 136, 27 / 2f)) {
            openedAdd = true;
            return true;
        }
        if (!openedAdd) {
        ...
        if (RenderUtils.isInRegion(mouseX, mouseY, xA + width - 45, y + 5, 39, 10)) {
            openedAdd = false;
            nameTyping = false;
            return true;
        }
        if (RenderUtils.isInRegion(mouseX, mouseY, xA + 15, y + 30, width - 25, 15)) {
            nameTyping = !nameTyping;
        }
        if (openedAdd) {
            if (RenderUtils.isInRegion(mouseX, mouseY, xA + 15, y + 50, width - 25, height - 20)) {
    ...
    private void saveItemsToFile(List<String> items) {
        try (FileWriter writer = new FileWriter("resolute/automist.epf")) {
