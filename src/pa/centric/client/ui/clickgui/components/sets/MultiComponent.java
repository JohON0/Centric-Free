...
public class MultiComponent extends Component {
    ...
    public MultiComponent(ModuleComponent object, MultiBoxSetting set) {
        this.object = object;
        this.set = set;
        setting = set;
    }
    ...
    public void draw(MatrixStack stack, int mouseX, int mouseY) {
        ...
        for (BooleanOption mode : set.options) {
            float preOffset = offset + FontUtils.sfbold[14].getWidth(mode.getName()) + 3;
            if (preOffset > size) {
                lines++;
                offset = 0;
            }
            offset += FontUtils.sfbold[14].getWidth(mode.getName()) + 3;
        }
        ...
        int i = 0;
        for (BooleanOption mode : set.options) {
            float preOff = offset + FontUtils.sfbold[14].getWidth(mode.getName()) + 3;
            if (preOff > size) {
                offset = 0;
                offsetY += 13;
            }
            if (set.get(i)) {
                RenderUtils.Render2D.drawGradientRound(x + 10.5f + offset, y + 13 + offsetY,FontUtils.sfbold[14].getWidth(mode.getName()),7, 2,
                ...
                RenderUtils.Render2D.drawGradientRound(x + 10.5f + offset, y + 13 + offsetY,FontUtils.sfbold[14].getWidth(mode.getName()),7f, 2, ColorUtil.rgba(35, 35, 35, 255),ColorUtil.rgba(35, 35, 35, 255),ColorUtil.rgba(35, 35, 35, 255),ColorUtil.rgba(35, 35, 35, 255));
            }
                FontUtils.sfbold[14].drawString(stack, mode.getName(), x + 10.5f + offset, y + 15 + offsetY, new Color(255, 255, 255, 255).getRGB());
            offset += FontUtils.sfbold[14].getWidth(mode.getName()) + 3;
            i++;
        }
    }
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        ...
        for (BooleanOption mode : set.options) {
            ...
            if (RenderUtils.isInRegion(mouseX, mouseY, x + 10.5f + offset, y + 12 + offsetY, FontUtils.sfbold[14].getWidth(mode.getName()), FontUtils.sfbold[14].getFontHeight() / 2f + 3)) {
                set.set(i, !set.get(i));
            }
