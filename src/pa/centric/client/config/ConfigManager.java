...
public final class ConfigManager {
    ...
    public static final File CONFIG_DIR = new File(Minecraft.getInstance().gameDir, "\\Centric\\Client_1_16_5\\configs");
    private final File autoCfgDir = new File(Minecraft.getInstance().gameDir, "\\Centric\\Client_1_16_5\\autocfg.cfg");
    private static final JsonParser jsonParser = new JsonParser();
    ConfigManager configManager = conduction.CONFIG_MANAGER;
...
//            loadConfiguration("autocfg", true);
    ...
    public void loadConfiguration(String configuration, boolean start) {
        ...
        if (config == null) {
            System.out.println(Centric.prefix + "Конфиг " + configuration + " не был найден!");
            return;
        }
        ...
        try {
            ...
            System.out.println(Centric.prefix + "Ошибка разбора JSON-строки.");
        } catch (NullPointerException e) {
            e.printStackTrace();
            System.out.println(Centric.prefix + "Фатальная ошибка конфига.");
            System.out.println(Centric.prefix + "Конфиг " + configuration + " устарел!");
        } catch (ClassCastException classCastException) {
            System.out.println(Centric.prefix + "Фатальная ошибка конфига.");
            System.out.println(Centric.prefix + "Конфиг " + configuration + " пустой!");
    ...
    public void deleteConfig(String configName) {
        ...
        if (config != null) {
            ...
            if (file.exists()) {
                ...
                if (!deleted) {
                    System.out.println(Centric.prefix + "Не удалось удалить конфиг " + file.getAbsolutePath());
                }
            }
        }
    }
