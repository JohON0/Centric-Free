...
@ModuleAnnotation(name = "ElytraFly", category = Type.Movement)
public class ElytraFly extends Function {
    ...
    private void noFireworks() {
        ClientUtils.sendMessage("Нету феерверков в инвентаре!");
        setState(false);
        onDisable();
        flying = false;
        ticksInAir = 0;
    ...
    private void noElytra() {
        ClientUtils.sendMessage("Нету элитр в инвентаре!");
        setState(false);
        onDisable();
        flying = false;
        ticksInAir = 0;
