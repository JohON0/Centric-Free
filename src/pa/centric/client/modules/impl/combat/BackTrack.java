...
@ModuleAnnotation(name = "BackTrack", category = Type.Combat)
    ...
    private void handleRenderEvent(EventRender e) {
        if (e.isRender3D() && target != null && draw.get()) {
            ...
            GlStateManager.enableBlend();
            {
                ...
                RenderUtils.Render3D.drawBox(target.getBoundingBox().offset(target.realXI, target.realYI, target.realZI).offset(-mc.getRenderManager().info.getProjectedView().x, -mc.getRenderManager().info.getProjectedView().y, -mc.getRenderManager().info.getProjectedView().z), -1);
            }
