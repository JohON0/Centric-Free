...
import pa.centric.client.modules.settings.imp.ColorSetting;
...
@ModuleAnnotation(name = "Tracers", category = Type.Render)
            ...
            if (eventRender.isRender3D()) {
                ...
                for (PlayerEntity entity : mc.world.getPlayers()) {
                    if (entity instanceof RemoteClientPlayerEntity && entity.botEntity && mc.gameSettings.getPointOfView() == PointOfView.FIRST_PERSON) {
                        int tracersColor = conduction.FRIEND_MANAGER.isFriend(entity.getName().getString()) ? friendColor.get() : color.get();
